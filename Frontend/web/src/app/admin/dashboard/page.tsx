...
function StatCard({ label, value, sub, color, Icon }: { label: string; value: string | number; sub?: string; color: string, Icon: any }) {
  return (
    <div style={{
      background: "var(--admin-panel-bg)", border: `1px solid var(--admin-border)`,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ color: "var(--admin-text-sub)", fontSize: 13, fontWeight: 600, letterSpacing: 0.5 }}>{label.toUpperCase()}</div>
      </div>
      <div style={{ color: "var(--admin-text-main)", fontSize: 42, fontWeight: 700, lineHeight: 1, zIndex: 1 }}>{value}</div>
      {sub && <div style={{ color: "var(--admin-text-muted)", fontSize: 13, marginTop: 12, zIndex: 1 }}>{sub}</div>}
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  return (
    <AdminShell>
      <div>
        <h1 style={{ color: "var(--admin-text-main)", fontSize: 28, fontWeight: 700, margin: "0 0 8px", letterSpacing: "-0.5px" }}>Dashboard</h1>
        <p style={{ color: "var(--admin-text-sub)", marginBottom: 40, fontSize: 15 }}>Real-time overview of the SentriZK authentication and threat ecosystem.</p>
        ...
        <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "stretch" }}>
            {/* Recent users table */}
            <div style={{ flex: 2, background: "var(--admin-panel-bg)", border: "1px solid var(--admin-border)", borderRadius: 16, overflow: "hidden", minWidth: 500, transition: "background 0.2s, border-color 0.2s" }}>
            <div style={{ padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 style={{ color: "var(--admin-text-main)", fontSize: 16, fontWeight: 600, margin: 0 }}>Recent Sign Ups</h2>
            ...
            {loading ? (
                ...
                <thead>
                    <tr style={{ background: "rgba(0,0,0,0.02)", borderTop: "1px solid var(--admin-border)", borderBottom: "1px solid var(--admin-border)" }}>
                    {["User", "Status", "Registered", "Last Login"].map(h => (
                        <th key={h} style={{ padding: "14px 24px", color: "var(--admin-text-muted)", fontSize: 12, fontWeight: 600, textAlign: "left", letterSpacing: 0.5 }}>{h.toUpperCase()}</th>
                    ))}
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={u.username} style={{ borderBottom: "1px solid var(--admin-border)", transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background = "var(--admin-panel-hover)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                        <td style={{ padding: "16px 24px", color: "var(--admin-text-main)", fontWeight: 600 }}>{u.username}</td>
                        ...
                        <td style={{ padding: "16px 24px", color: "var(--admin-text-sub)", fontSize: 13 }}>
            ...
            <div style={{ flex: 1, minWidth: 300, display: "flex", flexDirection: "column", gap: 16 }}>
                <Link href="/admin/threats" style={{ textDecoration: "none", flex: 1 }}>
                    <div style={{
                    height: "100%", background: "var(--admin-panel-bg)", border: "1px solid var(--admin-border)",
                    ...
                    onMouseLeave={e => { e.currentTarget.style.background = "var(--admin-panel-bg)"; e.currentTarget.style.borderColor = "var(--admin-border)"; }}
                    >
                    ...
                    <div style={{ color: "var(--admin-text-main)", fontWeight: 600, fontSize: 16, marginBottom: 4 }}>Review Threat Logs</div>
                    <div style={{ color: "var(--admin-text-sub)", fontSize: 14 }}>{threatCount} flagged messages require attention.</div>
                    </div>
                </Link>
                ...
                <Link href="/admin/users" style={{ textDecoration: "none", flex: 1 }}>
                    ...
                    <div style={{ color: "var(--admin-text-main)", fontWeight: 600, fontSize: 16, marginBottom: 4 }}>Manage Users</div>
                    <div style={{ color: "var(--admin-text-sub)", fontSize: 14 }}>Hold, restore, or revoke network access.</div>
                    </div>
                </Link>
            </div>
        </div>
