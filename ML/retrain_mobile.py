...
def run_mobile_training():
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_PATH = os.path.join(BASE_DIR, "DataSet", "train_ready.csv") 
    MODELS_DIR = os.path.join(BASE_DIR, "models")
    os.makedirs(MODELS_DIR, exist_ok=True)

    console.print(Panel("[bold cyan]SentriZK Mobile Engine (Pure TFLite)[/bold cyan]\n[dim]Architecture: Embedding + Conv1D + GlobalAvgPool[/dim]"))
    ...
    console.print("[blue]Converting to LiteRT (.tflite) with PURE TFLITE OPS ONLY...[/blue]")
    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    
    # 🌟 CRITICAL: ONLY BUILTINS. No Flex ops needed for CNNs.
